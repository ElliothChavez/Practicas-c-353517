...
int main() {
	...
	printf ("Desea agregar un  nuevo numero a la lista? (1=SI/0=NO)\n");
	scanf("%d",&resp);
	if(resp ==1) {
		printf("\nIngrese el elemento a agregar: ");
		scanf("%d",&e);
		insetarEnLista (&cabeza,e);
		printf("\nSe imprime la lista cargada: ");
		imprimirLista(cabeza);
	}
...
void insertarEnLista (tpuntero *cabeza, int e) {
	...
	nuevo = malloc(sizeof(tnodo)); /*Utilizamos malloc para reservar memoria para ese */
...
void BuscarEnLista(tpuntero *cabeza){
	while(*cabeza != NULL);{
